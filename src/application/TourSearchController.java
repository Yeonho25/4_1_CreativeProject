...
public class TourSearchController {
	...
	@FXML
	private TableView<Tour> addrTable;
	@FXML
	private TableColumn<Tour,String> colName;
	@FXML
	private TableColumn<Tour,String> colClassification;
	@FXML
	private TableColumn<Tour,String> colCapacity;
	@FXML
	private TableColumn<Tour,String> colAddress;
...
//		addrTable.getSelectionModel().selectedItejamProperty().addListener(
//				(observable,oldValue,newValue) -> showTourDetails((Tour) newValue));
...
//		                 System.out.println("Mouse Double Click!");
		        	  try
		      		{
		      			FXMLLoader loader = new FXMLLoader();
		      			  loader.setLocation(getClass().getResource("/view/TourInformation.fxml"));
		      			  Parent root = (Parent) loader.load();
		      			  Scene scene = new Scene(root);
		      			  
		      			  Tour_CRUD pop = loader.getController();
		      			  pop.initData(addrTable.getSelectionModel().getSelectedItem().getName());
	...
	public void changeRestaurantView() {
		try {
			...
			Stage primaryStage = (Stage) foodBtn.getScene().getWindow(); // 현재 윈도우 가져오기
	...
	private void showSearchTour(String siDo, String area) {
		try {
			data = (ObservableList<Tour>) tourDao.searchTour(tourName.getText(), siDo,area);
			
			colName.setCellValueFactory(new PropertyValueFactory<Tour,String>("name"));
			colClassification.setCellValueFactory(new PropertyValueFactory<Tour,String>("classification"));
			colCapacity.setCellValueFactory(new PropertyValueFactory<Tour,String>("capacity"));
			colAddress.setCellValueFactory(new PropertyValueFactory<Tour,String>("address"));
	...
	private void showAllTour() {
		try {
			data = (ObservableList<Tour>) tourDao.getTourList();
...
//		
//		try {
//			Parent tourDetail = FXMLLoader.load(getClass().getResource("TourInformation.fxml"));
//			Scene sc = new Scene(tourDetail);
